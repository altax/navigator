#!/bin/bash
# API server startup for the artifact workflow (port 8081).
# Runs independently from the main API Server workflow (port 8080).
set -euo pipefail
...
echo "[api-artifact] Checking dependencies..."
pnpm install --frozen-lockfile --prefer-offline 2>/dev/null \
  || pnpm install --frozen-lockfile
echo "[api-artifact] Dependencies OK."
